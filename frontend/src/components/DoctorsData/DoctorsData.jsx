...
const DoctorsData = () => {
    ...
    const handleChange = (e) => {
        ...
        if (type === 'checkbox') {
            if (name === 'agreementConsent' || name === 'feeVariesByComplexity' || name === 'availability24_7' || 
                name === 'onCallConsultation' || name === 'followUpDiscount' || name === 'referralProgram' ||
                name === 'connectedHospitalDatabase' || name === 'medicalRecordAccess' || 
                name === 'allowPatientReviews' || name === 'publicProfileVisibility') {
    ...
    const addDegree = () => {
        setFormData(prev => ({
            ...prev,
            degrees: [...prev.degrees, { degree: '', otherDegree: '' }],
        }));
    };

    const updateDegree = (index, field, value) => {
        const updatedDegrees = formData.degrees.map((deg, i) =>
    ...
    const addCertification = () => {
        setFormData(prev => ({
            ...prev,
            additionalCertifications: [...prev.additionalCertifications, ''],
        }));
    };
    ...
    const addClinicDetail = () => {
        setFormData(prev => ({
            ...prev,
            clinicDetails: [...prev.clinicDetails, { name: '', address: '', yearsOfStay: '' }],
        }));
    };

    const updateClinicDetail = (index, field, value) => {
        const updatedDetails = formData.clinicDetails.map((detail, i) =>
            i === index ? { ...detail, [field]: value } : detail
        );
        setFormData(prev => ({ ...prev, clinicDetails: updatedDetails }));
    };

    const removeClinicDetail = (index) => {
        setFormData(prev => ({
            ...prev,
            clinicDetails: prev.clinicDetails.filter((_, i) => i !== index),
        }));
    };
    ...
    const addWorkSchedule = () => {
        setFormData(prev => ({
            ...prev,
            workSchedule: [...prev.workSchedule, { day: '', timeSlots: [] }],
        }));
    };

    const updateWorkSchedule = (index, field, value) => {
        const updatedSchedule = formData.workSchedule.map((schedule, i) =>
            i === index ? { ...schedule, [field]: value } : schedule
        );
        setFormData(prev => ({ ...prev, workSchedule: updatedSchedule }));
    };

    const removeWorkSchedule = (index) => {
        setFormData(prev => ({
            ...prev,
    ...
    const toggleTimeSlot = (index, slot) => {
        ...
        if (timeSlots.includes(slot)) {
            updatedSchedule[index].timeSlots = timeSlots.filter(s => s !== slot);
        } else {
            updatedSchedule[index].timeSlots.push(slot);
        }
        setFormData(prev => ({ ...prev, workSchedule: updatedSchedule }));
    };

    const confirmUpload = (field) => {
        setFormData(prev => ({
            ...prev,
                            ...
                            {formData.degrees.map((deg, index) => (
                                <div key={index} className={styles.dynamicEntry}>
                                    ...
                                    <button
                                        ...
                                        disabled={formData.degrees.length === 1}
                                    >
                                        <FaTrash />
                                    </button>
                                </div>
                            ))}
                            <button type="button" onClick={addDegree} className={styles.addButton}>Add Another Degree</button>
                        </label>
                    </div>
                    <div className={styles.inputGroup}>
                        <label className={styles.inputLabel}>
                    ...
                    <div className={styles.inputGroup}>
                        <label className={styles.inputLabel}>
                            ...
                            >
                                ...
                                {/* Default for other specializations */}
                            ...
                            {formData.additionalCertifications.map((cert, index) => (
                                <div key={index} className={styles.dynamicEntry}>
                                    ...
                                    <button
                                        ...
                                        disabled={formData.additionalCertifications.length === 1}
                                    >
                                        <FaTrash />
                                    </button>
                                </div>
                            ))}
                            <button type="button" onClick={addCertification} className={styles.addButton}>Add Another Certification</button>
                        </label>
                    </div>
                    <div className={styles.inputGroup}>
                        <label className={styles.inputLabel}>
                            ...
                            {formData.clinicDetails.map((detail, index) => (
                                <div key={index} className={styles.dynamicEntry}>
                                    ...
                                    <button
                                        ...
                                        disabled={formData.clinicDetails.length === 1}
                                    >
                                        <FaTrash />
                                    </button>
                                </div>
                            ))}
                            <button type="button" onClick={addClinicDetail} className={styles.addButton}>Add Another Clinic/Hospital</button>
                        </label>
                    </div>
                    <div className={styles.inputGroup}>
                        ...
                        {formData.workSchedule.map((schedule, index) => (
                            <div key={index} className={styles.scheduleEntry}>
                                ...
                                <button
                                    ...
                                    disabled={formData.workSchedule.length === 1}
                                >
                                    <FaTrash />
                                </button>
                            </div>
                        ))}
                        <button type="button" onClick={addWorkSchedule} className={styles.addButton}>Add Another Schedule Entry</button>
                    </div>
                    <div className={styles.inputGroup}>
